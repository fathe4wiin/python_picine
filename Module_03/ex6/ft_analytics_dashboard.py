def main():
    ...
    high_scorers = [summoners[i] for i in range(len(scores)) if scores[i] > 2000]
    ...
    summoner_scores = {summoners[i]: scores[i] for i in range(len(summoners)) if scores[i] > 1700}
    ...
    achievement_count_map = {summoners[i]: achievement_counts[i] for i in range(len(summoners))}
    ...
    unique_achievements = {ach for ach_list in achievements for ach in ach_list}
